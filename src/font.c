...
static void cod_draw_char(cod_image* src, int src_x, int src_y, int width,
  ...
  for(int y = 0; y < height; y++) {
    ...
    for(int x = 0; x < width; x++) {
      ...
      // TODO: Fix this
      float tint_r = ((fg.r / 255.0f) * srcp->r);
      float tint_g = ((fg.g / 255.0f) * srcp->g);
      float tint_b = ((fg.b / 255.0f) * srcp->b);
      //int alpha = (srcp->r && srcp->g && srcp->b) ? srcp->a : 0;
...
static void cod_draw_char2(cod_image* src, int src_x, int src_y, int width,
                          int height, cod_image* dst, int dst_x, int dst_y,
                          cod_pixel fg) {
  assert(width && height);
...
void cod_draw_text2(cod_font* font, const char* text, cod_pixel fg,
  ...
  while((c = *text++)) {
    ...
    cod_draw_char2(font->image, ch->x, ch->y, ch->width, ch->height, target, dstx + x + ch->xoffset, dsty + ch->yoffset, fg);
